package de.fhkoeln.gm.eis.ss15.xmpp.connection;
	...
	public boolean publishItemPayload(String nodeId, String elemName, String payloadData) {
		...
		if (node != null) {
			try {
				...
				node.send(new PayloadItem(null, new SimplePayload("test", null, "<book xmlns='pubsub:test:book'><title>Lord of the Rings</title></book>")));
			} catch (XMPPException e) {
				e.printStackTrace();
	...
	public String getNodeInformation(String node_id) {
        ...
        try {
            ...
            while (it.hasNext()) {
                ...
                LeafNode node = (LeafNode) pubSubMngr.getNode(node_id);
